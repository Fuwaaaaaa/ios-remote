...
impl StatsExporter {
    ...
    pub fn push(&mut self, fps: f64, latency: f64, bandwidth: f64, frames: u64, cpu: f64, mem: f64) {
        self.samples.push(StatsSample {
            timestamp: Local::now().format("%Y-%m-%d %H:%M:%S").to_string(),
            fps, latency_ms: latency, bandwidth_kbps: bandwidth,
            frames_total: frames, cpu_percent: cpu, memory_mb: mem,
        });
    }

    pub fn export_csv(&self, path: &str) -> Result<(), String> {
        let mut csv = "timestamp,fps,latency_ms,bandwidth_kbps,frames_total,cpu_percent,memory_mb\n".to_string();
        for s in &self.samples {
            csv.push_str(&format!("{},{:.1},{:.1},{:.1},{},{:.1},{:.1}\n",
                s.timestamp, s.fps, s.latency_ms, s.bandwidth_kbps, s.frames_total, s.cpu_percent, s.memory_mb));
        }
