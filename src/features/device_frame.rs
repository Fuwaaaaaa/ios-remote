...
impl DeviceFrame {
    ...
    pub fn apply(&self, rgba: &[u8], w: u32, h: u32) -> (Vec<u8>, u32, u32) {
        ...
        let (bezel, corner, notch) = match &self.model {
            ...
            DeviceModel::Custom { corner_radius, bezel_width, notch } => (*bezel_width, *corner_radius, *notch),
        };

        let out_w = w + bezel * 2;
        let out_h = h + bezel * 2;
        let mut out = vec![0x22u8; (out_w * out_h * 4) as usize];
        for i in 0..(out_w * out_h) as usize { out[i * 4 + 3] = 255; }

        // Bezel color: dark gray with metallic look
        for y in 0..out_h { for x in 0..out_w {
            let idx = ((y * out_w + x) * 4) as usize;
            if idx + 2 >= out.len() { continue; }
            out[idx] = 0x33; out[idx + 1] = 0x33; out[idx + 2] = 0x38;
        }}
        ...
        if notch {
            ...
            let nw = 80u32;
            let nh = 20u32;
            for dy in 0..nh { for dx in 0..nw {
                let px = cx - nw / 2 + dx;
                let py = ny - nh / 2 + dy;
                let idx = ((py * out_w + px) * 4) as usize;
                if idx + 2 < out.len() {
                    out[idx] = 0x11; out[idx + 1] = 0x11; out[idx + 2] = 0x11;
                }
            }}
        }
