...
- 現在の screenshotr キャプチャは PNG→RGBA 経路で `Frame.h264_nalu` を埋めません。従って `SessionRecorder` が保存する `video.h264` は空で、再生しても描画は発生しません。H.264 ソースの接続は v0.6 以降のフォローアップです (`docs/ROADMAP-v0.6.md` 参照)
