...
35 個の Command Palette アクションを REST / Stream Deck / 内部ホットキーから同じ経路で起動できます。`/api/command/{id}` の HTTP ステータスでハンドラの状況がわかります:
...
v0.6 時点で **23/35 アクションが dispatch 完了**(screenshot / recording / OCR / AI describe / QR / zoom / game_mode / annotation_clear / web_dashboard / settings / firewall_setup / translate / startup_toggle / quit / color_pick ほか)。残りは Phase C 続編(annotation_*, ruler, privacy_*)と引数を要するもの(macro_run, lua_run, network_diag, gif_save)で、`409` を返して理由を明示します。
